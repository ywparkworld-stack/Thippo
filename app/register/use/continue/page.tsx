...
export default function RegisterUseContinuePage() {
    ...
    <div className="min-h-screen bg-gray-50 font-[family-name:var(--font-geist-sans)]">
      <nav className="fixed top-0 w-full bg-white border-b border-gray-100 z-50">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <span className="text-sm text-purple-100 bg-purple-600 px-3 py-1 rounded-full font-semibold">拝借様向け登録</span>
        </div>
      </nav>
      ...
      <div className="pt-24 pb-16 px-6 max-w-lg mx-auto">
            ...
            <div key={s.num} className="flex items-center">
              <div className="flex flex-col items-center">
                <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold transition-colors ${
                  ...
                  : step === s.num ? "bg-purple-600 text-white ring-4 ring-purple-100"
                  : "bg-gray-200 text-gray-400"
                }`}>
                  {step > s.num ? "✓" : s.num}
                </div>
                <span className={`text-xs mt-1 font-medium whitespace-nowrap ${step >= s.num ? "text-purple-600" : "text-gray-400"}`}>
                  {s.label}
                </span>
              </div>
              {i < steps.length - 1 && (
                <div className={`h-0.5 w-10 sm:w-14 mx-1 mb-4 transition-colors ${step > s.num ? "bg-purple-600" : "bg-gray-200"}`} />
              )}
            </div>
          ))}
        </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-8 shadow-sm">
            ...
            <div className="space-y-5">
              <div>
                ...
                <input type="password" placeholder="8文字以上" className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-purple-400 focus:border-transparent" />
              ...
              <div>
                ...
                <input type="password" placeholder="もう一度入力" className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-purple-400 focus:border-transparent" />
              </div>
            </div>
            <button onClick={() => setStep(2)} className="w-full mt-8 bg-purple-600 text-white py-4 rounded-xl font-semibold hover:bg-purple-700 transition-colors">
              次へ：身分証を登録する
            </button>
          </div>
        )}
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-8 shadow-sm">
                ...
                {[
                  ...
                  <button
                    ...
                    className={`flex items-center gap-3 p-4 rounded-xl border-2 text-sm font-medium transition-all text-left ${
                      idType === item.id ? "border-purple-500 bg-purple-50 text-purple-700" : "border-gray-200 text-gray-600 hover:border-purple-300"
                    }`}
                  >
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="border-2 border-dashed border-gray-200 rounded-xl p-8 text-center hover:border-purple-400 hover:bg-purple-50 transition-all cursor-pointer">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  顔写真（セルフィー）
                  <span className="ml-2 text-xs text-purple-600 font-normal">身分証と一緒に撮影してください</span>
                </label>
                <div className="border-2 border-dashed border-gray-200 rounded-xl p-8 text-center hover:border-purple-400 hover:bg-purple-50 transition-all cursor-pointer">
                  <span className="text-4xl block mb-2">🤳</span>
            ...
            <div className="flex gap-3 mt-8">
              ...
              <button onClick={() => setStep(3)} className="flex-[2] bg-purple-600 text-white py-4 rounded-xl font-semibold hover:bg-purple-700 transition-colors">次へ：支払い方法を登録する</button>
            </div>
          </div>
        )}
                ...
                {[
                  ...
                  <button
                    ...
                    className={`w-full flex items-center gap-4 p-4 rounded-xl border-2 text-left transition-all ${
                      payMethod === item.id ? "border-purple-500 bg-purple-50" : "border-gray-200 hover:border-purple-300"
                    }`}
                  >
                    <span className="text-3xl">{item.icon}</span>
                    <div>
                      <p className={`text-sm font-semibold ${payMethod === item.id ? "text-purple-700" : "text-gray-700"}`}>{item.label}</p>
                      <p className="text-xs text-gray-400">{item.desc}</p>
                    </div>
                    {payMethod === item.id && <span className="ml-auto text-purple-600 text-lg">✓</span>}
                  </button>
                ))}
              </div>
            </div>
            {payMethod === "card" && (
              <div className="space-y-4 p-5 bg-gray-50 rounded-xl border border-gray-100">
                <div>
                  ...
                  <input type="text" placeholder="1234 5678 9012 3456" className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-purple-400 bg-white" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1.5">有効期限</label>
                    <input type="text" placeholder="MM / YY" className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-purple-400 bg-white" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1.5">セキュリティコード</label>
                    <input type="text" placeholder="CVC" className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-purple-400 bg-white" />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1.5">カード名義</label>
                  <input type="text" placeholder="TARO YAMADA" className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-purple-400 bg-white" />
                </div>
              </div>
            )}
            <div className="flex gap-3 mt-8">
              ...
              <button onClick={() => setStep(4)} className="flex-[2] bg-purple-600 text-white py-4 rounded-xl font-semibold hover:bg-purple-700 transition-colors">登録を完了する</button>
            </div>
          </div>
        )}
        ...
        {step === 4 && (
          <div className="bg-white rounded-2xl border border-gray-100 p-10 shadow-sm text-center">
            ...
            <div className="bg-purple-50 rounded-xl p-4 text-sm text-purple-700 mb-8 text-left space-y-2">
              <p>✓ 基本情報の登録</p>
              <p>✓ 身分証のアップロード（審査中）</p>
              <p>✓ 支払い方法の登録</p>
            </div>
            <Link href="/use" className="block w-full bg-purple-600 text-white py-4 rounded-xl font-semibold hover:bg-purple-700 transition-colors">
              本日の空きスペースを見る
            </Link>
          </div>
        )}
